...
const evalLet = (exp: LetExp, env: Env): Result<Value> => {
    ...
    //let a 5, b lambda(x)... , let a b
    //store<- 5 , lambda
    //newEnv <- a, b , &5 &lambda(x)
...
const evalSet = (exp: SetExp, env: Env): Result<void| unknown>=>
    safe2((val: Value, addr: number) => addr!=-1 ? makeOk(setStore(theStore,addr,val)) : makeFailure("var not found"))
